...
export default function FeedbackModal() {
  ...
  const [feedback, setFeedback] = useLocalStorage(STORAGE_KEYS.feedback, [], {
    legacyKeys: [],
  ...
  async function copy() {
    try {
      await navigator.clipboard.writeText(buildPrompt(feedback));
      setCopied(true);
  ...
  return (
    <>
      <button
        ...
        aria-label="功能願望與回饋"
        className="fixed bottom-[5.5rem] right-4 z-30 flex h-12 w-12 items-center justify-center rounded-full bg-rose-deep text-white shadow-[var(--shadow-soft)] transition active:scale-90"
      >
      ...
      {open && (
        ...
        >
          <div className="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-[var(--shadow-soft)]">
            <div className="flex items-center justify-between border-b border-rose-soft/60 px-5 py-4">
            ...
            <form onSubmit={submit} className="space-y-3 px-5 py-4">
              <label className="block">
                ...
                <textarea
                  ...
                  required
                  maxLength={500}
                  placeholder="例如：新增扭蛋分類／夜間模式…"
                  className="w-full rounded-2xl border border-rose-soft bg-mist px-4 py-3 text-base outline-none ring-rose-brand focus:ring-2"
                />
              </label>
              <button type="submit" className="min-h-12 w-full rounded-2xl bg-rose-brand font-bold text-white shadow-[var(--shadow-soft)] transition active:scale-[0.98]">
                加入並產生 Cursor Prompt
              </button>
            </form>
            ...
            {feedback.length > 0 && (
              <div className="border-t border-rose-soft/60 px-5 py-4">
                ...
                <button
                  type="button"
                  onClick={copy}
                  className="mt-3 min-h-11 w-full rounded-2xl border border-rose-soft bg-mist text-sm font-bold text-ink transition active:scale-[0.98]"
                >
