(() => {
  ...
  const STORAGE_KEYS = {
    ...
    adapt: "thailand-adapt-weather",
    feedback: "thailand-feedback",
    evolution: "thailand-evolution",
  ...
  const WEATHER_POOL = [
    { label: "多雲", icon: "⛅", hi: 35, lo: 28, rain: 40 },
    { label: "午後雷陣雨", icon: "⛈️", hi: 34, lo: 27, rain: 75 },
    { label: "晴朗炎熱", icon: "☀️", hi: 38, lo: 29, rain: 15 },
    { label: "局部驟雨", icon: "🌦️", hi: 35, lo: 28, rain: 55 },
    { label: "陰天大雨", icon: "🌧️", hi: 32, lo: 26, rain: 90 },
  ];
  ...
  function weatherForDay(index) {
    const h = hashStr(`thailand-${index}`);
  ...
  function buildTip(weather, day) {
    ...
    const indoorText = indoors.length ? indoors.join("、") : "Siam／Central Rama 9 室內動線";
    if (weather.rainy) {
  ...
  function totalSpent() {
    return state.expenses.reduce((sum, e) => sum + (e.thb || 0), 0);
  }
  ...
  function budgetPace() {
    const remaining = Math.max(0, state.budgetThb - totalSpent());
  ...
  function renderBudgetAlert() {
    ...
    const avgPlanned = state.budgetThb / TRIP_DAYS;
    const overPace = spent > avgPlanned * (todayIndex() + 1) * 1.1;

    els.budgetAlert.classList.remove("hidden");
    if (overPace) {
      const lowCost = ["Lumphini Park 散步", "Chong Nonsi 巷弄 Cafe", "商場 Window Shopping", "住家附近公園"];
      els.budgetAlert.innerHTML = `
        <div class="flex items-start gap-2.5">
          <span class="text-lg" aria-hidden="true">⚠️</span>
          <div>
            <p class="font-display text-sm font-bold text-ink">預算優化提示</p>
            <p class="mt-0.5 text-[13px] leading-relaxed text-ink-soft">目前花費略超前；剩餘 ${formatThb(remaining)}，日均建議 ${formatThb(Math.round(perDay))}。今日可多排：${lowCost.join("、")}。</p>
          </div>
        </div>`;
      ...
      els.budgetAlert.innerHTML = `
        <div class="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p class="mt-0.5 text-[13px] text-ink-soft">已花 ${formatThb(Math.round(spent))}／${formatThb(state.budgetThb)} · 日均可用 ${formatThb(Math.round(perDay))}</p>
          </div>
        </div>`;
  ...
  const EVOLUTION_POOL = [
    { id: "thai-milk", name: "泰奶評分圖鑑", desc: "逐間記低泰奶甜度／茶味／CP 值，儲低曼谷最強一杯。" },
    { id: "coins", name: "泰銖硬幣辨識速查卡", desc: "1／2／5／10 銖硬幣圖鑑＋常見找續組合。" },
    { id: "cafe-wheel", name: "今日隨機選 Cafe 轉盤", desc: "由已記錄嘅足跡隨機抽一間，選擇困難救星。" },
    { id: "gacha-budget", name: "扭蛋式預算分配器", desc: "將今日剩餘預算隨機拆做「食／買／玩」三份，每日小驚喜。" },
    { id: "phrase", name: "泰語常用語速查卡", desc: "點餐／講價／多謝必備句型，附羅馬拼音，離線可用。" },
    { id: "grab", name: "Grab 車資估算筆記", desc: "記低常用路線實際車資，Call 車前心中有數。" },
    { id: "offline-map", name: "離線地圖下載提醒", desc: "出門前檢查：今日去嘅區域離線地圖下載咗未？" },
    { id: "steps", name: "每日步數目標追蹤", desc: "記低目標同實際步數，回望成就感十足。" },
    { id: "souvenir", name: "手信清單＋預算分配", desc: "邊個送邊份、預算幾多、買咗未，一頁睇晒。" },
    { id: "rabbit", name: "BTS Rabbit 卡餘額追蹤", desc: "記低每次增值同餘額，唔使去到閘口先發現唔夠錢。" },
    { id: "fx-alert", name: "泰銖匯率波動提醒", desc: "泰銖兌港元升穿／跌穿自設門檻時提示，唱錢更精明。" },
    { id: "luggage", name: "行李重量估算器", desc: "按已買物品估算回程行李重量，避免超重罰款。" },
    { id: "wifi-notes", name: "免費 Wi-Fi 熱點筆記", desc: "記低商場／Cafe／BTS 站嘅免費 Wi-Fi 名同連線方法。" },
    { id: "diary", name: "每日三行回顧日記", desc: "每晚三行：最正一刻／最伏一刻／聽日最期待。" },
    { id: "photo-quest", name: "拍照打卡任務進度", desc: "預設 9 宮格任務（Siam／夜市／Cafe…），儲齊召喚回憶。" },
    { id: "timezone", name: "泰港時差對照器", desc: "一眼睇到而家香港幾點，約人打電話啱時間。" },
    { id: "emergency", name: "緊急聯絡卡", desc: "泰國報警 191／救護 1669／保險 hotline 一頁收藏，離線可睇。" },
    { id: "rainy-planb", name: "下雨天 Plan B 清單", desc: "預設落雨替代動線（Siam 商圈室內／商場 Cafe），落雨唔使諗。" },
  ];

  function evoOrder() {
    const arr = Array.from({ length: EVOLUTION_POOL.length }, (_, i) => i);
    let seed = hashStr("thailand-evolution") || 1;
