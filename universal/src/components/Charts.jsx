export function DoughnutChart({ segments, size = 180, thickness = 22, centerLabel = "分類", formatValue }) {
        ...
        {segments.map((seg) => {
          ...
          return (
            <li key={seg.id} className="flex items-center justify-between gap-2 text-sm">
              ...
              <span className="text-ink-soft">{pct}% · {formatValue ? formatValue(seg.value) : seg.value}</span>
            </li>
          );
        })}
      </ul>
